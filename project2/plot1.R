...
plot1 <- function() {
  ...
  fName<-"plot1.png"
  gTitle<-"Total PM2.5 Emissions from All Sources in the United States \n from 1999 to 2008"
  gColour<-"red"
  xLabel<-"year"
  ...
  yrange <- c(0, max(data)+1000000) ## add an offset to max 
  ticks <- pretty(yrange)
  ...
  axis(2, at = ticks, labels = labels, las = 1, cex.axis=0.5, ylim=yrange) 
