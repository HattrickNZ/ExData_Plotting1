...
plot4 <- function() {
  ...
  fName<-"plot4.png"
  gTitle<-"Total PM2.5 Emissions from Coal in the \n United States from 1999 to 2008"
  gColour<-"red"
  xLabel<-"year"
  ...
  # just interested in coal 
  subsetSCC <- subset(SCC, grepl("coal", EI.Sector, ignore.case=TRUE), select=c(SCC, EI.Sector))
  
  # merging the data 
  MD = merge(NEI,subsetSCC,by.x="SCC",by.y="SCC",all=FALSE)
  ...
  axis(2, at = ticks, labels = labels, las = 1, cex.axis=0.6, ylim=yrange) 
